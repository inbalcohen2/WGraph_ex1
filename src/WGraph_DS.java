...
class WGraph_DS implements weighted_graph,Serializable {
		...
		public NodeInfo(node_info other) {
			...
			for(edges i: other.getEdges().values()) {
	...
	public boolean hasEdge(int node1, int node2) {
		...
		if(nodes_TheGragh_W.containsKey(node1) &&nodes_TheGragh_W.containsKey(node2)) {
			...
			return getNode(node1).getEdges().containsKey(node2);
	...
	public double getEdge(int node1, int node2) {
		if(hasEdge(node1,node2)) {
			return this.getNode(node1).getEdges().get(node2).getWeight();
		}
		return -1;
	}
	/**
	...
	public void connect(int node1, int node2, double w) {
		 ...
		 if (hasEdge(node1,node2)) {
				getNode(node1).getEdges().get(node2).setWeight(w);
				getNode(node2).getEdges().get(node1).setWeight(w);
				MC++;
		 }
		if(nodes_TheGragh_W.containsKey(node1)&&nodes_TheGragh_W.containsKey(node2)
			...
			MC++;
			edgeSize++;
			getNode(node1).getEdges().put(node2, new edges(w,getNode(node2)));
			getNode(node2).getEdges().put(node1, new edges(w,getNode(node1)));
		}
		}
    ...
    */
	...
	public Collection<node_info> getV(int node_id) {
		...
		for(edges i:getNode(node_id).getEdges().values()) {
	...
	public void removeEdge(int node1, int node2) {
		if(hasEdge(node1,node2)){
			MC++;
			edgeSize--;
			getNode(node1).getEdges().remove(node2);
			getNode(node2).getEdges().remove(node1);
		}
	}		
	...
	public boolean equals(Object obj) {
		return this.nodes_TheGragh_W.size()==((weighted_graph)obj).nodeSize()&&this.edgeSize==((weighted_graph)obj).edgeSize();
	}
